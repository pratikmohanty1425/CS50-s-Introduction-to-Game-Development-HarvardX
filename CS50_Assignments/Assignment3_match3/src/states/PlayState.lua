    ...
    for y=1,8 do 
        for x=1,8 do 
            if not matchfound then
                if self:foundmatches("up",x,y) or self:foundmatches("down",x,y) or self:foundmatches("left",x,y) or self:foundmatches("right",x,y)then
                    matchfound=true;
                end
            end
        end
    ...
    if self.canInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                --update 4
                --no swapping if no matches found
...
--update 4
--to check if any possible matches available
function PlayState:foundmatches(direction, x, y)
    local Ttile =self.board.tiles[y][x]
    local Ttempx=Ttile.gridX
    local Ttempy=Ttile.gridY
    local TNtile=nil

    if y>1 and direction=="up" then
        TNtile = self.board.tiles[y-1][x]
    elseif y<8 and direction=="down" then
        TNtile = self.board.tiles[y+1][x]
    elseif x>1 and direction=="left" then
        TNtile = self.board.tiles[y][x-1]
    elseif x<8 and direction=="right" then
        TNtile = self.board.tiles[y][x+1]
    else 
        return false
    end

    Ttile.gridX = TNtile.gridX
    Ttile.gridY = TNtile.gridY
    TNtile.gridX = Ttempx
    TNtile.gridY = Ttempy

    self.board.tiles[Ttile.gridY][Ttile.gridX] = Ttile
    self.board.tiles[TNtile.gridY][TNtile.gridX] = TNtile

    local Fmatches = self.board:calculateMatches()

    --revert back
    TNtile.gridX = Ttile.gridX
    TNtile.gridY = Ttile.gridY
    Ttile.gridX = Ttempx
    Ttile.gridY = Ttempy
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            ...
            for i, tile in pairs(match) do 
                VarietyBonus = VarietyBonus + (tile.variety * 5)
                if tile.shiny then
                    ShinyBonus = ShinyBonus + 50 * 7
                end
            end

            self.score = self.score + #match * 50 +VarietyBonus + ShinyBonus

            --Update 1 add time when match
